...
import { ICrew, ICrewMember } from "models/crew/crewModel.types";

dotenv.config()

export class CrewServiceFunctions extends ModelModule {
   private readonly db: typeof mongoose
   ...
   /**
      ...
      * @export
      */
   async updateCrew(referralCode: string, user: IUser) {
      ...
      while (currentRefCode && level <= 3) { // Set max depth if desired
         ...
         const levelKey = `level_${level}` as keyof ICrew;
         if (Array.isArray(referrerCrew[levelKey])) {
            (referrerCrew[levelKey] as ICrewMember[]).push(crewMember);
         } else {
   ...
   async getUserCrew(email: string) {
      const existingUser = await this.userModel.findOne({ email })
      if (!existingUser) throw new NotFoundError('User not found, Login please');
   ...
   async updateCrewOnTransaction(userID: string, transactionType: 'deposit' | 'withdraw', amount: number) {
      ...
      while (currentRefCode && level <= 10) {
         const referrer: IUser | null = await this.userModel.findOne({ referral_code: currentRefCode });
         if (!referrer) break;
         ...
         const levelKey = `level_${level}` as keyof ICrew;
         const crewMembers = (crew[levelKey] || []) as ICrewMember[];
  ...
  */
   async awardReferralBonus(userID: string, amount: number, bonusType: 'first_deposit' | 'mining_profit', coin: string = 'USDT') {
      ...
      while (currentRefCode && level <= 3) {
         ...
         const bonusTransaction = new this.userTransactionModel({
         ...
         await session.withTransaction(async () => {
            ...
            const rootUser =
               (await this.userModel
                  ...
                  .session(session)) as IUser | null;

            if (!rootUser) throw new NotFoundError('User not found');

            const rootCrew: ICrew | null = await this.crewModel
            ...
            if (deep && rootCrew) {
               ['level_1', 'level_2', 'level_3' as const].forEach((level) => {
                  (rootCrew[level as keyof ICrew] as ICrewMember[]).forEach(m =>
            ...
            for (const victimID of toDelete) {
               ...
               for (const c of crews) {
                  ...
                  (['level_1', 'level_2', 'level_3'] as const).forEach(lvl => {
                     const idx = (c[lvl] as ICrewMember[]).findIndex(
                        m => m.userID === victimID,
                     );
                     if (idx > -1) {
                        const [gone] = (c[lvl] as ICrewMember[]).splice(idx, 1);
                        // adjust roll-ups
                        c.totalMembers -= 1;
            ...
            await this.userTransactionModel.deleteMany(
               { userID: { $in: [...toDelete] } },
               { session },
            );
...
const CrewService = new CrewServiceFunctions()
