...
import { ICrewMember } from "models/crew/crewModel.types";
import { ModelModule } from "models/Model.module";

export class CrewExtraServiceFunctions extends ModelModule{
   constructor() {
      super();
   }
     ...
     */
         ...
         try {
            ...
            for (const userInfo of usersToDelete) {
               try {
                  ...
                  console.error(`❌ Error deleting user ${userInfo.username}:`, errMsg);
               }
            }
            ...
            const errMsg = error instanceof Error ? error.message : String(error);
            throw new BadRequestError(`Failed to perform deep deletion: ${errMsg}`);
         }
      }
      ...
      private async deleteSingleUserComplete(userID: string, username: string) {
         ...
         const transactionCount = await this.userTransactionModel.countDocuments({ email: user.email });
         await this.userTransactionModel.deleteMany({ email: user.email });
      ...
      private async removeUserFromAllCrews(userID: string, username: string) {
         ...
         for (const crew of crewsWithUser) {
            ...
            let removedMember: ICrewMember | null = null;
      ...
      async previewDeepDeletion(userID: string) {
         ...
         for (const userInfo of usersToDelete) {
            ...
            if (user) {
               const txCount = await this.userTransactionModel.countDocuments({ email: user.email });
...
const CrewExtraService = new CrewExtraServiceFunctions();
export default CrewExtraService;